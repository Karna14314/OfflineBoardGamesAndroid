...
/**
 * Dots & Boxes reducer — wraps core [Reducer] with game-specific state.
 */
class DotsAndBoxesReducer {

    private val rules = DotsAndBoxesRules()
    private val coreReducer = Reducer(rules)

    fun reduce(currentState: DotsAndBoxesState, intent: GameIntent): Pair<DotsAndBoxesState, List<GameEffect>> {
        return when (intent) {
            is GameIntent.RestartGame -> {
                ...
                val freshState = DotsAndBoxesState(
                    gameState = freshGame,
                    vsAi = currentState.vsAi,
    ...
    companion object {
        fun createInitialGameState(
            ...
            return GameState(
                ...
                boardData = DotsAndBoxesRules.createInitialBoard(),
